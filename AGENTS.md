...
`/root/Hermes/projects/sd-tinder-backend-mvp-v2026.06.30.1` — mounted at the **same absolute path** on the host and inside the container. **Do all
...
- **Never relocate work to `/tmp`, bare `/root`, `~`, or any non-mounted path.** Those are container-local
  ...
  `/root/Hermes/projects/sd-tinder-backend-mvp-v2026.06.30.1` persist.
...
workspace (`--workspace dir:/root/Hermes/projects/sd-tinder-backend-mvp-v2026.06.30.1`). The verifier **gates** every milestone: it passes only on
...
- **The sandbox has no Docker.** `docker compose up/exec` and curls to compose-published host ports cannot be
  ...
  the host e2e loop is what proves the compose path.
