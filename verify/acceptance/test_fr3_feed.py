...
def test_feed_returns_candidates_sorted_by_distance(client, fresh_user_id):
    ...
    create_profile(
        client, far_id, name="FarAway",
        gender="men", age=25,
        lat=40.8000, lon=-74.0060,
    )

    # Medium: ~5km away
    mid_id = "mid-user-00000000000000000003"
    create_profile(
        client, mid_id, name="Middle",
        gender="men", age=25,
        lat=40.7578, lon=-74.0060,
    )
...
def test_feed_excludes_self(client, fresh_user_id):
    ...
    create_profile(
        client, requester_id,
        name="Requester", gender="women", age=25,
        lat=40.7128, lon=-74.0060,
        preferences={"gender": "men", "age_min": 18, "age_max": 99, "radius_km": 100},
    )
    ...
    assert requester_id not in candidate_ids, \
...
def test_feed_respects_gender_preference(client):
    ...
    create_profile(
        client, woman_id,
        name="Woman", gender="women", age=25,
        lat=40.7128, lon=-74.0060,
    )
    ...
    assert woman_id not in candidate_ids, "Woman should NOT appear (gender preference mismatch)"
...
def test_feed_respects_age_range(client):
    ...
    create_profile(
        client, too_young_id,
        name="TooYoung", gender="men", age=20,
        lat=40.7128, lon=-74.0060,
    )
    ...
    assert too_young_id not in candidate_ids, "Age 20 should be excluded from 25-30 range"
...
def test_feed_empty_when_no_candidates(client, fresh_user_id):
    ...
    create_profile(
        client, requester_id,
        name="Lonely", gender="women", age=25,
        lat=0.0, lon=0.0,  # middle of ocean, no neighbors
